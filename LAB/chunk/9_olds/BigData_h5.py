...
def from_plink(
    ...
    categorical_phenotype: bool, True by default
        ...
        start_variants: incluido na pesquisa
        end_variante: nao esta incluido na pesquisa
    ...
    v_variables = len(variant_info.index)
    v_cicles = math.ceil(v_variables / 32000)  # SQLite sotrage capacity

    # v_cicles = 3

    dp_chunk = pd.DataFrame(columns=["start", "stop"])
    v_start = 0
    v_stop = 0
    ...
    if v_cicles > 1:
        for i in range(0, v_cicles):
            v_start = v_stop
            v_stop = v_stop + 32000
            dp_chunk.loc[len(dp_chunk.index)] = [v_start, v_stop]
    else:
        dp_chunk.loc[len(dp_chunk.index)] = [0, v_variables]

    for index, row in dp_chunk.iterrows():
        ...
        variant_info = variant_info.iloc[row["start"] : row["stop"]]

        variant_list = [create_variant(row) for idx, row in variant_info.iterrows()]
        # print(f"\tLoaded information for {len(variant_list)} variants from '{bim_file.name}'")
        ...
        df_x = pd.concat([df, pd.DataFrame.from_dict(gt_array_dict)], axis=1)
        df_x = df_x.set_index(
...
if __name__ == "__main__":
    ...
    result = from_plink(input, categorical_phenotype=True)

    result.to_csv("~/Public/fromPlink_teste1.txt", sep=";")

    # result.to_hdf("~/Public/fromPlink_teste1.h5", "df_x", mode="w", format="table")
    print(result.info())
